...
之后执行 Data Generation 和 Train Model 命令：[do.sh](do.sh)。除来原本 `run.sh` 中的部分，还添加了这几轮试验中发现需要的一些辅助命令。

此外，因为  Train Model 输出过多且耗时很长，故而将输出（包括错误输出）重定向到文件中，且使用 time 计时。
...
此次 Train Model 输出太长。故而未列出。这里列出一条重复出现的 DEBUG 警告输出。
