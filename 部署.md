...
之后执行 Data Generation 和 Train Model 命令：[do.sh](do.sh)。先创建 tmux 会话，激活环境，再运行脚本。

```shell
tmux new -s train
pyenv activate apenv
wget https://github.com/xuanhao44/SUAPCD/blob/main/do.sh
./do.sh
```
...
执行下面命令，第二条的 `<username>` 和 `<password>` 改成自己的。

```shell
wget https://github.com/xuanhao44/SUAPCD/blob/main/pre_neo.sh
