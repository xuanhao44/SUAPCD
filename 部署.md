...
之后执行 Data Generation 和 Train Model 命令：[do.sh](do.sh)。

- 除来原本 `run.sh` 中的部分，还添加了这几轮试验中发现需要的一些辅助命令。

-  因为  Train Model 输出过多且耗时很长，故而：
  - 后台运行。参考：<https://zhuanlan.zhihu.com/p/617627144>
  - 将输出（包括错误输出）重定向到文件中。
  - 使用 time 计时。
...
本次试采用 tensorflow 的新安装方式：它能一起安装除了显卡驱动以外的程序。（具体版本可能和上面描述的略的不同）
