...
2. 运行其代码需要加上 `PYTHONPATH=$(pwd)` 的前缀。原因在 [3.2 Data Generation 部分](# 3.2-Data-Generation-部分) 中有写。
...
没有使用 `pyenv-installer` 的原因是由于不知名的网络问题，安装过程可能会出现问题，且不好排查问题并继续安装。后续可考虑使用简易安装。
