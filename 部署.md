...
命令在 [prepare.sh](prepare.sh) 中。

- 需要提前将用户名和密码写入其中。
- 命令执行顺序和前面可能有不一样的地方。建议先检查一遍。
...
环境配置仍使用 [prepare.sh](prepare.sh)，未改变内容。

之后执行 Data Generation 和 Train Model 命令：[do.sh](do.sh)。除来原本 `run.sh` 中的部分，还添加了这几轮试验中发现需要的一些辅助命令。此外，因为  Train Model 输出过多且耗时很长，故而重定向到文件中，且使用 time 计时。
...
2. augmentation_lib.py:187 else 输出：初步鉴定为正常输出。
...
Train Model 输出：[2023.11.27_ModelTrain_Out](2023.11.27_ModelTrain_Out.txt)
